...
Antes de elegir ejercicios, mira "## BLOQUE ACTUAL (EL QUE SE SUSTITUYE)" —el plan que está siguiendo ahora mismo y al que este bloque sucede—, "## ENTRENOS RECIENTES" (qué entrena ya y con qué cargas) y "## MESOCICLOS ANTERIORES" (qué splits ha seguido): la continuidad vale más que un diseño nuevo cada bloque, así que mantén lo que le funciona y cambia lo que tenga motivo. Si cambias la estructura respecto al bloque actual, di por qué en "notes". Si no aparece "## BLOQUE ACTUAL", es que no hay ninguno en curso: no supongas uno. Si la petición trae equipamiento, no prescribas nada que requiera material fuera de esa lista. Ten en cuenta también las notas del perfil: recogen preferencias, disponibilidad y contexto que el perfil no formaliza.
